...
public class TokenRepository extends SQLiteOpenHelper implements Repository<String, Token> {
	...
	private static enum Table {
		...
		TOKEN( "CREATE TABLE %s ( treasury TEXT NOT NULL, id TEXT, secret TEXT NOT NULL, value INTEGER, PRIMARY KEY (treasury,id) );", "treasury", "id", "secret", "value" );
	...
	public void delete( final Token token ) {
		...
		database.delete( Table.TOKEN.name(), "treasury = ? AND id = ?", new String [] { token.getTreasury(), token.getId() } );
	...
	public Token findById( String id ) {
		...
		Cursor cursor = database.query( Table.TOKEN.name(), Table.TOKEN.fields, "id = ?", new String [] { id }, null, null, null );
		Token token = null;
	...
	private SimpleToken getTokenFromCurrentRow( Cursor cursor ) {
		return new SimpleToken( cursor.getString( 0 ), cursor.getString( 1 ), cursor.getString( 2 ), cursor.getInt( 3 ) );
	}
	...
	private ContentValues toContentValues( Token token ) {
		...
		values.put( "secret", token.getSecret() );
